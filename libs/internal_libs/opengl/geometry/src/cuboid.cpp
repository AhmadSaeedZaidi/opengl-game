...
OpenGL::Geometry::Cuboid::Cuboid(const glm::vec3& position, const glm::vec3& size,
    ...
    : position_(position),
      ...
      capsTexID(0) {
  updateModelMatrix();
  std::cout << "Cuboid constructor called" << std::endl;
}
...
void OpenGL::Geometry::Cuboid::rotate(float angle, const glm::vec3& axis) {
...
void OpenGL::Geometry::Cuboid::translate(const glm::vec3& offset) {
  position_ += offset;
  updateModelMatrix();
}
