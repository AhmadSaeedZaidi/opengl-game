...
Game::Game(int width, int height, const char* title, const char* vertexShader,
           const char* fragmentShader)
    : WindowApp(width, height, title),
...
void Game::addShape(std::unique_ptr<Shape> shape) { shapes_.emplace_back(std::move(shape)); }
...
void Game::processInput() {
  ...
  WindowApp::processInput();

  // Camera input is handled in update()
}

void Game::setupUniforms() {
  static int count = 0;
